...
class Gridworld(mdp.MarkovDecisionProcess):
    ...
    def getTransitionStatesAndProbs(self, state, action): # state와 action을 넣으면 next_state와 next_state에 갈 확률이 나오는 함수
        """
        ...
        if action == 'north' or action == 'south':
            if action == 'north':
                successors.append((northState,1-self.noise)) # 확률이 1-self.noise 
            else:
                successors.append((southState,1-self.noise))

            massLeft = self.noise
            successors.append((westState,massLeft/2.0)) # action이 위아래더라도 noise 만큼은 옆으로 빠지게 하자~
        ...
        if action == 'west' or action == 'east':
            ...
            massLeft = self.noise
            successors.append((northState,massLeft/2.0)) # action이 좌우더라도 noise 만큼은 위아래로 빠지게 하자 ~~~~~~~~~~~~~~
