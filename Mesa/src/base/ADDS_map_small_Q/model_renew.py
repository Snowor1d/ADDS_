    ...
    def floyd_warshall(self): #공간과 공간사이의 최단 경로를 구하는 알고리즘 
        ...
        for start in self.space_graph.keys():
            for end in self.space_graph[start]:
                ...
                middle_goal = space_connected_linear(start, end_t)
                distance_matrix[start][end_t] = math.sqrt(pow(start_xy[0]-middle_goal[0], 2) + pow(start_xy[1]-middle_goal[1], 2)) + math.sqrt(pow(end_xy[0]-middle_goal[0], 2)+pow(end_xy[1]-middle_goal[1], 2))
