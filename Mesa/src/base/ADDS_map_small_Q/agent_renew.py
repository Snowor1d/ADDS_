...
class FightingAgent(Agent):
    ...
    def robot_policy2(self):
        ...
        print("UP : ", self.distance_F(0, "UP", "guide"))
        print("DOWN : ", self.distance_F(0, "DOWN", "guide"))
        print("LEFT : ", self.distance_F(0, "LEFT", "guide"))
        print("RIGHT : ", self.distance_F(0, "RIGHT", "guide"))
        return (next_x, next_y)
    ...
    def distance_F(self, state, action, mode):
        ...
        for i in evacuation_points: #space_target에서 가장 가까운 탈출구를 찾기 
            if(floyd_distance[((now_space[0][0],now_space[0][1]), (now_space[1][0], now_space[1][1]))][i]<min_distance):
                exit = i 
