...
class FightingModel(Model):
    ...
    def __init__(self, number_agents: int, width: int, height: int):
        ...
        self.random_agent_distribute_outdoor(200)
        ...
        if(self.is_left_exit):
            self.space_goal_dict[((0,0), (35, 65))] = [self.left_exit_goal]
        ...
        for i in range(len(space)):
            if (self.only_one_wall[space[i][0]][space[i][1]] == 1 and space[i][0]!=0 and space[i][1]!=0 and space[i][1]!=199):
                continue
            c = FightingAgent(100000+i, self, space[i], 12)
            self.schedule_w.add(c)
    ...
    def SKKU_jagua(self):
            ...
            room_index = [3, 4, 7, 8, 11, 13, 15, 17, 23, 27, 29, 31, 33, 36, 37, 40, 42]
            
            for i in range(len(room_index)):
                room_num = space[room_index[i]-1]
                self.room_list.append([[room_num[0][0], room_num[0][1]], [room_num[1][0], room_num[1][1]]])
            for i in space:
                self.space_list.append([[i[0][0], i[0][1]], [i[1][0], i[1][1]]])
    ...
    def way_to_exit(self):
        if(self.is_left_exit):
            for i in self.space_graph[((0,0), (35, 65))]:
                self.space_goal_dict[((i[0][0], i[0][1]), (i[1][0], i[1][1]))] = [goal_extend(((i[0][0], i[0][1]), (i[1][0], i[1][1])), space_connected_linear(i, [[0,0], [35, 65]]))]
        #if(self.is_right_exit):
    ...
    def random_agent_distribute_outdoor(self, agent_num):
        ...
        for i in range(space_num - 1):
            ...
            while(random_num>space_agent*(1/5)):
                random_num = random.randint(1, space_agent - sum(random_list) - (space_num - i - 1))
    ...
    def init_outside(self): #외곽 탈출로 구현 
        ...
        self.space_type[((0,0), (35, 65))] = 0
        self.space_index[((0,0), (35, 65))] = 1
        self.space_list.append([[0,0], [35,65]])

        self.space_type[((0, 65), (5,195))]= 0
        self.space_index[((0, 65), (5, 195))] = 1
        self.space_list.append([[0,65], (5,195)])

        self.space_type[((0, 195), (199, 199))] = 0
        self.space_index[((0, 195), (199, 199))] = 1
        self.space_list.append([[0, 195], [199, 199]])

        #self.space_goal_dict[((0,195), (195, 199))] = [[0,0]]
        self.space_type[((195,5), (199, 195))] = 0
        self.space_index[((195,5), (199, 195))] = 2
        self.space_list.append([[195, 5], [199, 195]]) 
        ...
        self.space_type[((35, 0), (199, 5))] = 0
        self.space_index[((35, 0), (199, 5))] = 4
        self.space_list.append([[35, 0], [199, 5]])
