...
## 여기 아래에서 weight 변경하는 코드 넣어주세용
    ...
    def F1_distance(self, state, action, mode):
        ...
        if(tuple(map(tuple, now_space)) == exit):
            goal_x = (self.model.exit_rec[0][0] + self.model.exit_rec[1][0]) / 2
            goal_y = (self.model.exit_rec[1][0] + self.model.exit_rec[1][1]) / 2
            next_goal[0] = goal_x
            next_goal[1] = goal_y
