...
class FightingAgent(Agent):
    ...
    def __init__(self, unique_id, model, pos, type): 
        ...
        self.agent_judge_probability = [random.gauss(80, 15)/100, random.gauss(60, 20)/100] #[로봇을 따라갈 확률, 다른 agent를 따라갈 확률]
    ...
    def which_goal_agent_want(self):
        ...
        if(self.previous_stage != now_stage): # stage가 바뀜
            mode_change = True #영역이 바뀌면, agent 모드가 다시 설정되어야 함
        ...
        if(mode_change):
            if(robot_d < robot_radius and robot_status == 1):
                ...
                if(random_value < self.agent_judge_probability[0]):
                    ...
                    if(len(to_follow) > 1 and random.random() > self.agent_judge_probability[1]):
                        self.type = 2     # 다른 agent를 따라가는 모드
                    else:
                        self.type = 1 # agent 자신의 길을 가는 모드
            else:
                if(len(to_follow) > 1 and random.random() > self.agent_judge_probability[1]):
                    self.type = 2
                else:
