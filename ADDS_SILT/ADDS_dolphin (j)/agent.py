...
def space_connected_linear(xy1, xy2): ## 두 공간을 넣으면 맞닿은 면의 중간 점 리턴 ## xy1 = [((x10, y10), (x11, y11))], xy2 = [((x20, y20), (x21, y21))]
    check_connection = []
    for i1 in range(51):
        tmp = []
        for j1 in range(51):
...
## 여기 아래에서 weight 변경하는 코드 넣어주세용
    ...
    def F0_distance(self, state, action, mode):
        ...
        re = floyd_distance[((now_space[0][0],now_space[0][1]), (now_space[1][0], now_space[1][1]))][exit] \
            - math.sqrt(pow(now_space_x_center-next_goal[0],2)+pow(now_space_y_center-next_goal[1],2))\
            + math.sqrt(pow(next_goal[0]-next_robot_position[0],2)+pow(next_goal[1]-next_robot_position[1],2))
    ...
    def select_Q(self, state) :
        ...
        for j in range(len(action_list)):
            ...
            if random.random() <= exploration_rate:
                actions = [["UP", "GUIDE"], ["UP", "NOGUIDE"], ["DOWN", "GUIDE"], ["DOWN", "NOGUIDE"], ["LEFT", "GUIDE"], ["LEFT", "NOGUIDE"], ["RIGHT", "GUIDE"], ["RIGHT", "NOGUIDE"]]
                selected = random.choice(actions)
            
            self.now_action = selected
        return selected 
    ...
    def calculate_Q(self, state, action):
        ...
        f0 = self.F0_distance(state, action[0], action[1])
        f1 = self.F1_near_agents(state, action[0], action[1])
        direction_agents_num = self.four_direction_compartment()
        f3 = self.F3_direction_agents(state, action[0], action[1], direction_agents_num)
        Q= 0
        if(action[1] == "GUIDE"):
            Q = f0 * self.feature_weights_guide[0] + f1*self.feature_weights_guide[1] + f3 * self.feature_weights_guide[2]
        else:
            Q = f0 * self.feature_weights_not_guide[0] + f1*self.feature_weights_not_guide[1] + f3*self.feature_weights_not_guide[2]
    ...
    def update_weight(self,reward):  
        ...
        f0 = self.F0_distance(robot_xy, self.now_action[0], self.now_action[1])
        f1 = self.F1_near_agents(robot_xy, self.now_action[0], self.now_action[1])
    ...
    #         if i not in pp_list:
    #             pp_list.append(i)
    #             self.dfs(i, self.model.space_graph[i], pp_list)
    #             pp_list.pop()
    ...
    #             if [[0, 0], [5, 45]] in self.model.space_graph[key]:
    #                 dict_NoC[key] = 1

    #         if (self.model.is_up_exit):
    #             dict_NoC[((0, 45), (45, 49))] = "X" 
    #             if [[0, 45], [45, 49]] in self.model.space_graph[key]:
    #                 dict_NoC[key] = 1
        
    #         if (self.model.is_right_exit):
    #             dict_NoC[((45, 5), (49, 49))] = "X" 
    #             if [[45, 5], [49, 49]] in self.model.space_graph[key]:
    #                 dict_NoC[key] = 1

    #         if (self.model.is_down_exit):
    #             dict_NoC[((5, 0), (49, 5))] = "X" 
    #             if [[5, 0], [49, 5]] in self.model.space_graph[key]:
    #                 dict_NoC[key] = 1
    ...
    #         if val == 0:
    #             p_list = [key]
    #             self.dfs(key, self.model.space_graph[key], p_list) #튜플, 리스트, 리스트 속 튜플
