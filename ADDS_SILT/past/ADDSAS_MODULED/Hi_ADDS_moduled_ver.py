...
from agent_HI import FightingAgent
...
import agent_HI
from agent_HI import WallAgent
import random
import copy
import math
import numpy as np
...
if visualization_mode == 'off':
    ...
    for i in range(n): # 에피소드 n번 돌린다
        ...
        print('num_remained_agent',agent_HI.num_remained_agent)
    #-----------------------------------------------------------------------------------------------------------------------
        if i == 0: # 처음 생성된 agent 수 저장
            num_assigned_agent = agent_HI.num_remained_agent

        if agent_HI.num_remained_agent <= int(num_assigned_agent*0.5): # 50% 이상 빠져나가면 그때 에피소드 수 저장
            if num_escaped_episodes["50%"] == 0:
                num_escaped_episodes["50%"] = i+1

        if agent_HI.num_remained_agent <= int(num_assigned_agent*0.2): # 80% 이상 빠져나가면 그때 에피소드 수 저장
            if num_escaped_episodes["80%"] == 0:
                num_escaped_episodes["80%"] = i+1
        
        if agent_HI.num_remained_agent == 0: # 모두 빠져나가면 그때 에피소드 수 저장 , 텍스트 파일에 저장
            ...
            agent_HI.num_remained_agent = 0 # 초기화
    #-----------------------------------------------------------------------------------------------------------------------
...
from model_HI import FightingModel
