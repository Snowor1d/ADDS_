...
for j in range(run_iteration):
    ...
    if visualization_mode == 'off':
        ...
        #----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            s_model_r.step()
            with open("basic_robot_map_5_third.txt", "a") as f:
                num_escaped_agents = 15 - s_model_r.num_remained_agent+1
                f.write("{},{}\n".format(i, num_escaped_agents))
            print('num_remained_agent',s_model_r.num_remained_agent)
        ...
        #-----------------------------------------------------------------------------------------------------------------------
            ...
            if s_model_r.num_remained_agent == 1: # 모두 빠져나가면 그때 에피소드 수 저장 , 텍스트 파일에 저장
