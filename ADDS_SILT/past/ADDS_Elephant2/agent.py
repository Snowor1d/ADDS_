...
class FightingAgent(Agent):
    ...
    def move(self) -> None:
        ...
        if (self.type == 3):
            ...
            #new_position = self.model.robot_respawn()
            reward = self.reward_distance(self.robot_xy, "none", "none")
            #print("reward : ", reward)
            self.reward_difficulty_space(self.robot_xy, "none", "none") ###여기야!!!!!!! 여기다가 reward 계산해야댕
...
## 여기 아래에서 weight 변경하는 코드 넣어주세용
    ...
    def robot_policy2(self):
        ...
        self.robot_space = self.model.grid_to_space[int(round(self.robot_xy[0]))][int(round(self.robot_xy[1]))] #로봇이 어느 stage에 있는지 나온다 
    ...
    def four_direction_compartment(self):
        ...
        for i in only_space:
            ...
            #print(next_vertex_matrix[now_s][key], "로 가기 위한 next_goal : ", next_goal)
            ...
            for m in four_actions:
                if (m=="UP"):
                    up_direction = math.sqrt(pow(next_goal[0]-robot_position[0],2)+pow(next_goal[1]-(robot_position[1]+one_foot),2))        
                elif (m=="DOWN"):
                    down_direction = math.sqrt(pow(next_goal[0]-robot_position[0],2)+pow(next_goal[1]-(robot_position[1]-one_foot),2))        
                elif (m=="LEFT"):
                    left_direction = math.sqrt(pow(next_goal[0]-(robot_position[0]-one_foot),2)+pow(next_goal[1]-robot_position[1],2))     
                elif (m=="RIGHT"):
                    right_direction = math.sqrt(pow(next_goal[0]-(robot_position[0]+one_foot),2)+pow(next_goal[1]-robot_position[1],2))     
